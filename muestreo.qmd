...
```{r}
tibble(est= te$replicates) %>% ggplot()+ geom_histogram(aes(x=est), bins=20, fill='purple', color='white')+theme_light()+xlab("Réplicas ingreso per cápita")+ggtitle("Distribución réplicas del ingreso per cápita estimado, Jkn")
```
...
```{r}
tibble(est= te$replicates) %>% ggplot()+ geom_histogram(aes(x=est), bins=20, fill='purple', color='white')+theme_light()+xlab("Réplicas ingreso per cápita")+ggtitle("Distribución réplicas del ingreso per cápita estimado, Rao-Wu")
```
...
Los dominios de estimación en este caso son no planeados, ya que como no se conoce a priori el factor de multiplicidad de las UPMs, tampoco se puede conocer ni controlar el tamaño de muestra, algunas UPMs de la muestra original podrían no estar en las réplicas y otras estarán repetidas.
